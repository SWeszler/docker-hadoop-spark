...
These are practical exercises from [Spark in Action, 2nd Edition by Jean-Georges Perrin](https://www.manning.com/books/spark-in-action-second-edition), which you can run on the CDEP Hadoop-Spark-Hive environment.
The data does not need to be put on HDFS. However, in these exercises, we will do so.

Once you have done that, you can copy the Spark commands consecutively, run them, and see how they work.
...
## Preparing files
Copy the directory with data to the namenode and copy them from there to HDFS

```
docker ps |grep namenode  # find the Container ID of the namenode
...
In this chapter, we do a small transformation: we concatenate the lastname and firstname.
...
In this chapter, we rename some columns from a CSV. We drop a few columns from the result and we add an id column, based on state, county, and datasetId.

### Renaming, removing and adding columns
...
Renaming columns and removing the OBJECTID, PERMITID, and GEOCODESTATUS columns from our dataframe.
```
...
We can save the dataframe as json.
```
...
Adjusting column names and removing columns so that a union is possible.
```
...
This first practical exercise is about the fact that Spark is "fundamentally lazy". You can perform transformations on your data, but as long as you do not request the result, Spark will not start working.
...
df =  df.withColumn("avg", expr("(lcl+ucl)/2")) \
        ...
        .withColumn("ucl2", col("ucl"))
```
With explain, you can see how Spark operated. This can be useful for debugging your Spark code.
```
# Step 6 - explain
df.explain()
