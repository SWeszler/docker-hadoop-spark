...
- `docker run --rm --network docker-hadoop-spark_default ... log-analyzer-scala`: submit the log analyzer job.
- `make build`: build the base Hadoop service images.
- `make wordcount`: build and run the sample Hadoop WordCount flow.
